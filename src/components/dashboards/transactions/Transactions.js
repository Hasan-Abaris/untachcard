...
const Transactions = () => {
    ...
    const inquryGet = async () => {
        ...
        try {
            ...
            const res = await axios.get(`${base_url}card-transaction/transaction`, {
                    ...
                    <table className="min-w-full bg-white">
                        <thead>
                            <tr className="text-gray-600 uppercase text-sm leading-normal bg-gray-100">
                                ...
                                <th className="py-3 px-6 text-center">Billing Type</th>
                                <th className="py-3 px-6 text-center">Date</th>
                                <th className="py-3 px-6 text-center">Status</th>
                                <th className="py-3 px-6 text-center">Invoice
                                </th>
                            ...
                            {inquiryData.length > 0 ? (
                                inquiryData.map((item, index) => (
                                    <tr key={index} className="text-center">
                                        <td>{item?.planId?.title || "N/A"}</td>
                                        <td>

                                            {item?.Amount}

                                        </td>
                                        <td>{item?.planId?.billing_type}</td>
