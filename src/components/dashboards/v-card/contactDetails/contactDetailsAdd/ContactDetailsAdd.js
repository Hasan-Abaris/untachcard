...
const ContactDetailsAdd = ({ isOpen, onClose, editCard }) => {
  // ✅ Hooks must always be at the top (not conditional)
  ...
  const getIdData = async (id) => {
    try {
      ...
      const res = await axios.get(
        `https://onlineparttimejobs.in/api/card-fields/${id}`,
        {
  ...
  // ✅ Hook always runs, conditions inside are fine
  useEffect(() => {
    if (editCard) {
  ...
  // ✅ After hooks — conditional render is now safe
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (editCard?._id) {
        res = await axios.put(
          `https://onlineparttimejobs.in/api/card-fields/user/update/${editCard._id}`,
          formData,
          { headers: { Authorization: `Bearer ${token}` } }
        );
      } else {
        res = await axios.post(
          `https://onlineparttimejobs.in/api/card-fields/user/add`,
          formData,
          { headers: { Authorization: `Bearer ${token}` } }
        );
      }
        ...
        <div className="space-y-5">
                        ...
                        {filteredIcons.slice(0, 60).map((icon) => {
                          ...
                          return (
                            <button
                              ...
                              className={`p-2 border rounded-md hover:bg-pink-100 flex justify-center items-center ${
          ...
          <div className="flex justify-end pt-4">
            <button
              ...
              disabled={loader}
            >
              {loader ? "Saving..." : editCard ? "Update" : "Create"}
            </button>
          </div>
        </div>
      </div>
