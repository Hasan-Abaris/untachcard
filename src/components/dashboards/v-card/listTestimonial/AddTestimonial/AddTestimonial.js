...
const AddTestimonial = ({ isOpen, onClose, onSubmit, editCard }) => {
    ...
    const handleSubmit = async () => {
        ...
        if (editCard?._id) {
            try {
                ...
                const res = await axios.put(`https://onlineparttimejobs.in/api/card-testimonials/user/update/${editCard?._id}`, formData, {
            ...
            try {
                const token = window.localStorage.getItem("token");
                const res = await axios.post(`https://onlineparttimejobs.in/api/card-testimonials/user/add`, formData, {
                    headers: { Authorization: `Bearer ${token}` },
                })
                // dispatch(fetchUseCard());
    ...
    const getIdData = async (id) => {
        try {
            const token = window.localStorage.getItem("token");
            const res = await axios.get(`https://onlineparttimejobs.in/api/card-testimonials/${id}`, {
                headers: { Authorization: `Bearer ${token}` },
            })
            // console.log(res);
