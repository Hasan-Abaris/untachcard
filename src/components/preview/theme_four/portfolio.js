...
>
  <div className="portfolio-list grid gap-6 justify-center auto-rows-auto" style={{ gridTemplateColumns: 'repeat(auto-fit, minmax(250px, 1fr))', maxWidth: '1200px', width: '100%' }}>
    {data.map((p) => (
      <div
        key={p._id}
      ...
      >
        <h3 className="text-lg font-semibold mb-2 text-center" style={{ color: cardData?.card_font_color || "#000000" }}>
          {p.title}
        </h3>
        <p className="mb-2 text-center" style={{ color: cardData?.card_font_color || "#333333", opacity: 0.9 }}>
          {p.description}
        </p>
        {p.image && (
          <img src={p.image} alt={p.title} className="rounded-lg w-full h-48 object-cover mb-2" />
        )}
        {p.url && (
          <a
            ...
            style={{ color: "#2563eb", fontFamily: cardData?.card_font || "inherit" }}
          >
            Visit
          </a>
        )}
